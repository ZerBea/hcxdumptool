...
if((fd_epoll= epoll_create(1)) < 0)
	{
	#ifdef HCXDEBUG
	fprintf(fh_debug, "epoll_create failed: %s\n", strerror(errno));
	#endif
	return false;
	} 
...
while(!wanteventflag)
	...
	if(epret == -1)
		...
		if(errno != EINTR)
			{
			#ifdef HCXDEBUG
			fprintf(fh_debug, "epooll_pwait failed: %s\n", strerror(errno));
			#endif
			errorcount++;
			}
		continue;
...
fprintf(stdout, "%s %s  (C) %s ZeroBeat\n"
	...
	"-----------------------\n"
	"Before you start %s\n"
	" 1. always get general information about the target (ESSID, CHANNEL, MAC_AP. IN/OFF RANGE)\n"
	"  %s --rcascan=active -F\n"
	" 2. always get more information about the behavior of the target (CLIENTS, FRAME TYPES)\n"
	"  tshark\n"
	" 3. always design a filter tailored to the target and compile it\n"
	"  %s --bpfc\n"
	" 4. always test the filter\n"
	"  tshark -f\n"
	" 5. always apply the designed filter\n"
	"  %s --bpf\n"
	" Further information:\n" 
	"  see man bpfc\n"
	"  see docs/example.md\n"
...
fprintf(stdout, "--ftc            : enable fake time clock\n"
	...
	"                       columns mode 1-3:\n"
	"                        CHA|LAST|EA123P|MAC-CL|MAC-AP|ESSID          (SCAN:  frequency/channel)\n"
	"                       column EA123P\n"
...
fprintf(stdout, "less common options:\n--------------------\n"
	...
	"                             default: %d seconds\n"
	"                             to disable the watchdog set 0 (this is not recommended)\n",
